...
export default function ServicesPage() {
  ...
  function openEdit(service) {
    ...
    setForm({ name: service.name, duration_minutes: service.duration_minutes, price: service.price || '', color: service.color || '#C2185B' })
    setShowForm(true)
  }
  ...
  async function toggleActive(e, service) {
    e.stopPropagation()
    try {
      const res = await api.put(`/api/services/${service.id}`, { ...service, is_active: service.is_active ? 0 : 1 })
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <div>
          <h1 className="text-xl md:text-2xl font-bold text-slate-800">שירותים</h1>
          {!loading && <p className="text-xs text-slate-400 mt-0.5">{services.length} שירותים</p>}
        </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-3.5">
                <div>
                  ...
                  <input type="text" value={form.name}
                    ...
                    className={inputClass} placeholder="עיצוב גבות" required />
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input type="number" value={form.duration_minutes}
                      ...
                      className={inputClass} min="5" max="480" required />
                  </div>
                  <div>
                ...
                {editingService && (
                  <button type="button" onClick={() => handleDelete(editingService.id)}
      ...
      {loading ? (
        ...
        <div className="text-center py-20 bg-white rounded-2xl border border-slate-100">
          ...
          <p className="text-slate-500 font-medium text-sm">אין שירותים עדיין</p>
          <p className="text-xs text-slate-400 mt-1">לחץ על "שירות חדש" כדי להתחיל</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 gap-3">
          {services.map(service => (
            <div key={service.id}
              className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 flex items-center gap-3 hover:shadow-md transition-shadow active:scale-[0.99]"
              onClick={() => openEdit(service)}
            >
              <div className="w-11 h-11 rounded-xl flex-shrink-0 shadow-sm" style={{ backgroundColor: service.color || '#C2185B' }} />
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <h3 className="font-semibold text-slate-800 text-sm">{service.name}</h3>
                  {!service.is_active && (
                    <span className="text-[10px] bg-slate-100 text-slate-400 px-2 py-0.5 rounded-full font-medium">לא פעיל</span>
                  )}
                </div>
                <p className="text-xs text-slate-400 mt-0.5">
                  {service.duration_minutes} דקות{service.price ? ` · ₪${service.price}` : ''}
                </p>
              </div>
              <button
                onClick={e => toggleActive(e, service)}
                className="text-xs text-slate-400 hover:text-slate-600 px-3 py-1.5 rounded-lg hover:bg-slate-50 transition font-medium flex-shrink-0"
              >
